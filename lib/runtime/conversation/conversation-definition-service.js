...
/**
 * Maintainer/tooling hook only.
 *
 * `_validateConversationDefinitions` is exported so wrapper-side bundle
 * validation can reuse the same conversation binding, loading, and evaluator
 * readiness checks as runtime code.
 *
 * Production runtime code must not call this hook directly. In particular,
 * command handlers, conversation orchestration, and other gameplay/runtime
 * paths should use:
 * - `getConversationDefinitionService(state)` for shared service access
 * - `service.getConversationDefinitionForNpc(npc, area)` to load an NPC's
 *   conversation definition
 * - `service.resolveConversationBinding(...)` / `service.loadConversationDefinition(...)`
 *   only when working at the lower-level service boundary
 *
 * Agents and maintainers must treat this export as bundle-validation-only.
 * If new production code seems to need this hook, that is a sign the higher-
 * level service API should be extended instead.
 */
module.exports = {
