 ...
 * @returns {*[]}
 */
function getStateEntryActions(state) {
  const onEntry = state && isObjectRecord(state.onEntry) ? state.onEntry : null;
  return cloneActions(onEntry && onEntry.actions);
}
 ...
 * @param {string} eventId
 * @param {Record<string, *>} eventDef
 * @returns {{ ok: true, transition: { target: string, actions: *[], index: number | null } | null } | ConversationRuntimeFailure}
 */
function selectEventTransition(input, trace, stateId, eventId, eventDef) {
  ...
  if (transitions) {
    for (let index = 0; index < transitions.length; index += 1) {
      ...
      return {
        ...
        transition: {
          target,
          actions: cloneActions(transition.actions),
          index,
        },
      };
    }
  ...
  return {
    ...
    transition: {
      ...
      actions: cloneActions(eventDef.actions),
      index: null,
    },
  };
}
...
function selectDefaultTransition(input, trace, stateId, state) {
  ...
  return {
    ...
    transition: {
      target,
      actions: cloneActions(defaultEvent.actions),
      index: null,
    },
  };
}
 ...
 * @returns {{ ok: true, settledState: string, visibleEvents: VisibleEvent[], stateEntryActions: *[] } | ConversationRuntimeFailure}
 */
function settleFromDestination(input, trace, destinationStateId) {
  /** @type {string[]} */
  const visitedStates = [];
  /** @type {*[]} */
  const stateEntryActions = [];
  ...
  while (true) {
    ...
    stateEntryActions.push(...getStateEntryActions(state));
...
function evaluateConversationRuntime(input) {
  ...
  /** @type {{ target: string, actions: *[], index: number | null } | null} */
  ...
  return {
    ...
    stateEntryActions: settled.stateEntryActions,
    trace,
  };
}
