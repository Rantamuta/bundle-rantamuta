...
const { validateAuthoredEffects } = require('../authored-instructions');
 ...
 * @returns {void}
 */
function appendEffectValidationErrors(errors, effects, sourceLabel) {
  const validation = validateAuthoredEffects(effects, { source: sourceLabel });
  if (validation.ok) {
    return;
  }
...
function validateConversationDefinition(doc, sourceLabel = '') {
  ...
  for (const [stateId, stateDef] of Object.entries(states)) {
    ...
    const onEntry = isObjectRecord(stateDef) && isObjectRecord(stateDef.onEntry)
      ? stateDef.onEntry
      : null;
    if (onEntry && Object.prototype.hasOwnProperty.call(onEntry, 'effects')) {
      appendEffectValidationErrors(errors, onEntry.effects, sourceLabel);
    }
    ...
    for (const [eventId, eventDef] of Object.entries(events)) {
      ...
      if (eventId === 'default') {
        ...
        if (Object.prototype.hasOwnProperty.call(eventDef, 'effects')) {
          appendEffectValidationErrors(errors, eventDef.effects, sourceLabel);
        }
        continue;
      }
      ...
      for (const transition of transitions) {
        ...
        if (Object.prototype.hasOwnProperty.call(transition, 'effects')) {
          appendEffectValidationErrors(errors, transition.effects, sourceLabel);
        }
