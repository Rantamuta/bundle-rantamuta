...
const { deleteWorldMetadataPath, getOrCreateWorldMetadataValuesRoot } = require('./world-metadata-service');
 ...
 * - Maintainer note: when adding or materially changing a mutation instruction
 *   contract here, check whether the authored-instructions transposer and validator
 *   need a corresponding update so authored DSL support stays aligned.
 *
 * Error contract:
...
/**
 * @typedef {NoopInstruction | TransferItemInstruction | MovePlayerInstruction | DoorMutationInstruction | OpenDoorInstruction | CloseAndLockDoorInstruction | SetPlayerMetadataInstruction | SetRoomMetadataInstruction | SetAreaMetadataInstruction | SetWorldMetadataInstruction | DeleteRoomMetadataInstruction | DeleteAreaMetadataInstruction | DeleteWorldMetadataInstruction} MutationInstruction
 */
 ...
 */
function setPlayerMetadata(instruction) {
...
/**
 * Ensure entity metadata values root exists for rollback restoration.
 *
 * @param {*} entity
 * @returns {Record<string, *>}
 */
function getOrCreateEntityMetadataValuesRoot(entity) {
  if (!entity || typeof entity !== 'object') {
    throw new TypeError('deleteMetadata.undo target entity must be an object.');
  }

  if (!isObjectRecord(entity.metadata)) {
    entity.metadata = {};
  }

  const metadata = /** @type {Record<string, *>} */ (entity.metadata);
...
/**
 * Restore a previously deleted metadata value by re-resolving the parent path.
 * Missing intermediate objects are re-created during rollback.
 *
 * @param {Record<string, *>} valuesRoot
 * @param {string[]} segments
 * @param {*} previousValue
 */
function restoreDeletedMetadataPath(valuesRoot, segments, previousValue) {
  const leafKey = segments[segments.length - 1];
  /** @type {Record<string, *>} */
  let cursor = valuesRoot;
  ...
  for (const segment of segments.slice(0, -1)) {
    ...
    cursor = /** @type {Record<string, *>} */ (cursor[segment]);
  }

  cursor[leafKey] = snapshotValueForUndo(previousValue);
}

/**
 * @param {SetRoomMetadataInstruction} instruction
 * @returns {function(): void}
 */
function setRoomMetadata(instruction) {
  const actor = instruction && instruction.actor;
  const room = actor && typeof actor === 'object' && actor.room && typeof actor.room === 'object'
    ? actor.room
    : null;
  if (!room) {
    throw new TypeError('setRoomMetadata.actor must resolve to actor.room.');
  }

  const segments = parseMetadataPathWithPattern(instruction && instruction.key, METADATA_SEGMENT_PATTERN);
  if (!segments) {
    throw new SyntaxError('setRoomMetadata.key must be a non-empty safe camelCase dot path.');
  }

  if (!Object.prototype.hasOwnProperty.call(instruction, 'value') || instruction.value === undefined) {
    throw new TypeError('setRoomMetadata.value must not be undefined.');
  }

  if (!isJsonSafeValue(instruction.value)) {
    throw new TypeError('setRoomMetadata.value must be JSON-safe.');
  }
  ...
  if (hadValuesProperty && !isObjectRecord(metadata.values)) {
    throw new TypeError('setRoomMetadata.path contains non-object metadata.values root.');
  }
  ...
  const metadataRootAfterWrite = metadata;
  const valuesRootAfterWrite = valuesRoot;
  const leafKey = segments[segments.length - 1];
  /** @type {Record<string, *>} */
  let cursor = valuesRoot;
  /** @type {Array<{ parent: Record<string, *>, segment: string }>} */
  ...
  for (const segment of segments.slice(0, -1)) {
    ...
    const nextCursor = cursor[segment];
    if (!isObjectRecord(nextCursor)) {
      throw new TypeError('setRoomMetadata.path contains non-object intermediate segment.');
    }
  ...
  if (hadLeaf && isObjectRecord(previousLeaf) && Object.keys(previousLeaf).length > 0) {
    throw new TypeError('setRoomMetadata.path conflicts with existing metadata subtree.');
  }

  cursor[leafKey] = nextValue;

  const roomLabel = entityRef(room) || 'unknown-room';
  logMutationVerbose(`ROOMMETA: Setting room [${roomLabel}] key [${segments.join('.')}]`);

  return () => {
    const valuesRootForUndo = getOrCreateEntityMetadataValuesRoot(room);

    if (hadLeaf) {
      restoreDeletedMetadataPath(valuesRootForUndo, segments, previousLeaf);
    } else {
      /** @type {* | Record<string, *>} */
      let undoCursor = valuesRootForUndo;
      for (const segment of segments.slice(0, -1)) {
        if (!isObjectRecord(undoCursor)) {
          break;
        }

        const nextCursor = undoCursor[segment];
        if (!isObjectRecord(nextCursor)) {
          undoCursor = null;
          break;
        }
    ...
    if (!hadValuesProperty && isObjectRecord(metadata.values)) {
      ...
      if (currentValuesRoot === valuesRootAfterWrite && Object.keys(valuesRootAfterWrite).length === 0) {
        delete metadata.values;
      }
    }

    if (!hadMetadataObject && isObjectRecord(room.metadata)) {
      const currentMetadataRoot = room.metadata;
      if (currentMetadataRoot === metadataRootAfterWrite && Object.keys(metadataRootAfterWrite).length === 0) {
 ...
 * @returns {function(): void}
 */
function setAreaMetadata(instruction) {
  const actor = instruction && instruction.actor;
  const room = actor && typeof actor === 'object' && actor.room && typeof actor.room === 'object'
    ? actor.room
    : null;
  ...
  return () => {
    const valuesRootForUndo = getOrCreateEntityMetadataValuesRoot(area);
    ...
    if (!hadMetadataObject && isObjectRecord(area.metadata)) {
      const currentMetadataRoot = area.metadata;
      if (currentMetadataRoot === metadataRootAfterWrite && Object.keys(metadataRootAfterWrite).length === 0) {
 ...
 * @param {SetWorldMetadataInstruction} instruction
 * @returns {function(): void}
 */
function setWorldMetadata(instruction, state) {
  const stateRecord = /** @type {Record<string, *>} */ (state);
  const segments = parseMetadataPathWithPattern(instruction && instruction.key, METADATA_SEGMENT_PATTERN);
  if (!segments) {
    throw new SyntaxError('setWorldMetadata.key must be a non-empty safe camelCase dot path.');
  }

  if (!Object.prototype.hasOwnProperty.call(instruction, 'value') || instruction.value === undefined) {
    throw new TypeError('setWorldMetadata.value must not be undefined.');
  }

  if (!isJsonSafeValue(instruction.value)) {
    throw new TypeError('setWorldMetadata.value must be JSON-safe.');
  }

  const hadMetadataProperty = Object.prototype.hasOwnProperty.call(stateRecord, 'metadata');
  const hadMetadataObject = isObjectRecord(stateRecord.metadata);
  const previousMetadata = stateRecord.metadata;
  const metadataBefore = hadMetadataObject
    ? /** @type {Record<string, *>} */ (stateRecord.metadata)
    : null;
  const hadValuesProperty = metadataBefore
    ? Object.prototype.hasOwnProperty.call(metadataBefore, 'values')
    : false;
  const hadValuesObject = metadataBefore
    ? isObjectRecord(metadataBefore.values)
    : false;
  const previousValuesRoot = metadataBefore
    ? metadataBefore.values
    : undefined;

  const nextValue = cloneJsonSafeValue(instruction.value);
  const valuesRoot = getOrCreateWorldMetadataValuesRoot(state);
  const metadata = /** @type {Record<string, *>} */ (stateRecord.metadata);
  const metadataRootAfterWrite = metadata;
  const valuesRootAfterWrite = valuesRoot;
  const leafKey = segments[segments.length - 1];
  /** @type {Record<string, *>} */
  ...
  for (const segment of segments.slice(0, -1)) {
    ...
    const nextCursor = cursor[segment];
    if (!isObjectRecord(nextCursor)) {
      throw new TypeError('setWorldMetadata.path contains non-object intermediate segment.');
    }
  ...
  const previousLeaf = hadLeaf
    ...
    : undefined;
  if (hadLeaf && isObjectRecord(previousLeaf) && Object.keys(previousLeaf).length > 0) {
    throw new TypeError('setWorldMetadata.path conflicts with existing metadata subtree.');
  }

  cursor[leafKey] = nextValue;
  logMutationVerbose(`WORLDMETA: Setting world key [${segments.join('.')}]`);

  return () => {
    const currentValuesRoot = getOrCreateWorldMetadataValuesRoot(state);
    if (hadLeaf) {
      restoreDeletedMetadataPath(currentValuesRoot, segments, previousLeaf);
      return;
    }

    /** @type {Record<string, *>} */
    let undoCursor = currentValuesRoot;
    for (const segment of segments.slice(0, -1)) {
      if (!isObjectRecord(undoCursor[segment])) {
        undoCursor[segment] = {};
      }
      undoCursor = /** @type {Record<string, *>} */ (undoCursor[segment]);
    }
    ...
    if (!hadValuesObject && isObjectRecord(metadata.values)) {
      const currentValuesRoot = metadata.values;
      if (currentValuesRoot === valuesRootAfterWrite && Object.keys(valuesRootAfterWrite).length === 0) {
        if (hadValuesProperty) {
    ...
    if (!hadMetadataObject && isObjectRecord(stateRecord.metadata)) {
      const currentMetadataRoot = stateRecord.metadata;
      if (currentMetadataRoot === metadataRootAfterWrite && Object.keys(metadataRootAfterWrite).length === 0) {
        if (hadMetadataProperty) {
          stateRecord.metadata = previousMetadata;
        } else {
          delete stateRecord.metadata;
        }
      }
    }
  };
 ...
 * @returns {function(): void}
 */
function deleteRoomMetadata(instruction) {
  const actor = instruction && instruction.actor;
  const room = actor && typeof actor === 'object' && actor.room && typeof actor.room === 'object'
    ? actor.room
    : null;
  if (!room) {
    throw new TypeError('deleteRoomMetadata.actor must resolve to actor.room.');
  }
  ...
  return () => {
    const valuesRoot = getOrCreateEntityMetadataValuesRoot(room);
    restoreDeletedMetadataPath(valuesRoot, segments, previousValue);
  };
}
 ...
 * @returns {function(): void}
 */
function deleteAreaMetadata(instruction) {
  const actor = instruction && instruction.actor;
  const room = actor && typeof actor === 'object' && actor.room && typeof actor.room === 'object'
    ? actor.room
    : null;
  ...
  return () => {
    const valuesRoot = getOrCreateEntityMetadataValuesRoot(area);
    restoreDeletedMetadataPath(valuesRoot, segments, previousValue);
  };
}
...
/**
 * @param {GameState} state
 * @param {DeleteWorldMetadataInstruction} instruction
 * @returns {function(): void}
 */
function deleteWorldMetadata(instruction, state) {
  const segments = parseMetadataPathWithPattern(instruction && instruction.key, METADATA_SEGMENT_PATTERN);
  if (!segments) {
    throw new SyntaxError('deleteWorldMetadata.key must be a non-empty safe camelCase dot path.');
  }

  const force = parseDeleteForce(instruction, 'deleteWorldMetadata');

  const deleteResult = deleteWorldMetadataPath(
    state,
    segments,
    force,
    deleteMetadataPath
  ...
  logMutationVerbose(`WORLDMETA: Deleting world key [${segments.join('.')}]`);
 ...
 * @returns {function(): void}
 */
function movePlayer({ player, toRoom, direction, suppressRoomBroadcast }) {
 ...
 * @returns {function(): void}
 */
function openDoor(instruction, state) {
  return operateDoor({
    ...instruction,
    type: 'operateDoor',
    mutation: 'open',
 ...
 * @returns {function(): void}
 */
function closeAndLockDoor(instruction, state) {
  return operateDoor({
    ...instruction,
    type: 'operateDoor',
    mutation: 'closeAndLock',
  }, state, 'closeAndLockDoor');
}
 ...
 * @returns {function(): void}
 */
function operateDoor(instruction, state, warningPrefixOverride) {
  if (!instruction || typeof instruction !== 'object' || typeof instruction.mutation !== 'string') {
    throw new TypeError('operateDoor.mutation is required.');
  }
  ...
  if (!result || result.ok !== true) {
    const warningPrefix = warningPrefixOverride || `operateDoor(${instruction.mutation})`;
...
function applyMutationInstruction(state, instruction) {
  ...
  switch (instruction.type) {
    ...
    case 'transferItem': return transferItem(instruction);
    case 'movePlayer': return movePlayer(instruction);
    case 'operateDoor': return operateDoor(instruction, state);
    case 'openDoor': return openDoor(instruction, state);
    case 'closeAndLockDoor': return closeAndLockDoor(instruction, state);
    case 'setPlayerMetadata': return setPlayerMetadata(instruction);
    case 'setRoomMetadata': return setRoomMetadata(instruction);
    case 'setAreaMetadata': return setAreaMetadata(instruction);
    case 'setWorldMetadata': return setWorldMetadata(instruction, state);
    case 'deleteRoomMetadata': return deleteRoomMetadata(instruction);
    case 'deleteAreaMetadata': return deleteAreaMetadata(instruction);
    case 'deleteWorldMetadata': return deleteWorldMetadata(instruction, state);
