...
const METADATA_TARGET_FIELDS = ['actor', 'player', 'roomRef'];

const METADATA_TARGETING_MATRIX = {
  setPlayerMetadata: ['player'],
  setRoomMetadata: ['actor', 'roomRef'],
  setAreaMetadata: ['actor'],
  setWorldMetadata: [],
  deleteRoomMetadata: ['actor', 'roomRef'],
  deleteAreaMetadata: ['actor'],
  deleteWorldMetadata: [],
};
 ...
 * @param {Record<string, *>} payload
 * @param {string} instructionName
 * @param {readonly string[]} supportedFields
 * @param {import('./contracts').AuthoredInstructionsFinding[]} errors
 * @param {string | undefined} source
 */
function validateMetadataTargeting(payload, instructionName, supportedFields, errors, source) {
  const supportedFieldSet = new Set(supportedFields);

  for (const field of supportedFields) {
    validateOptionalTextField(payload, field, errors, source, instructionName);
  }

  for (const field of METADATA_TARGET_FIELDS) {
    if (payload[field] === undefined || supportedFieldSet.has(field)) {
      continue;
    }
    ...
    pushError(
      ...
      `${instructionName}.${field} is not supported for this instruction.`,
      source,
      {
        instructionName,
        field,
 ...
 * @param {string | undefined} source
 */
function validateMetadataSet(payload, instructionName, requireActor, supportedFields, errors, source) {
  if (requireActor && supportedFields.includes('actor')) {
    requireTextField(payload, 'actor', errors, source, instructionName);
  }

  validateMetadataTargeting(payload, instructionName, supportedFields, errors, source);
 ...
 * @param {string | undefined} source
 */
function validateMetadataDelete(payload, instructionName, requireActor, supportedFields, errors, source) {
  if (requireActor && supportedFields.includes('actor')) {
    requireTextField(payload, 'actor', errors, source, instructionName);
  }
...
const VALIDATORS = {
  ...
  setPlayerMetadata(payload, errors, source) {
    validateMetadataSet(
      ...
      'setPlayerMetadata',
      false,
      METADATA_TARGETING_MATRIX.setPlayerMetadata,
      errors,
      source
    );
  },
  setRoomMetadata(payload, errors, source) {
    validateMetadataSet(
      ...
      'setRoomMetadata',
      false,
      METADATA_TARGETING_MATRIX.setRoomMetadata,
      errors,
      source
    );
  },
  setAreaMetadata(payload, errors, source) {
    validateMetadataSet(
      ...
      'setAreaMetadata',
      false,
      METADATA_TARGETING_MATRIX.setAreaMetadata,
      errors,
      source
    );
  },
  setWorldMetadata(payload, errors, source) {
    validateMetadataSet(
      ...
      'setWorldMetadata',
      false,
      METADATA_TARGETING_MATRIX.setWorldMetadata,
      errors,
      source
    );
  },
  deleteRoomMetadata(payload, errors, source) {
    validateMetadataDelete(
      payload,
      'deleteRoomMetadata',
      false,
      METADATA_TARGETING_MATRIX.deleteRoomMetadata,
      errors,
      source
    );
  },
  deleteAreaMetadata(payload, errors, source) {
    validateMetadataDelete(
      ...
      'deleteAreaMetadata',
      false,
      METADATA_TARGETING_MATRIX.deleteAreaMetadata,
      errors,
      source
    );
  },
  deleteWorldMetadata(payload, errors, source) {
    validateMetadataDelete(
      ...
      'deleteWorldMetadata',
      false,
      METADATA_TARGETING_MATRIX.deleteWorldMetadata,
      errors,
      source
    );
  },
