 ...
 * @param {string} effectName
 */
function validateOptionalTextField(payload, field, errors, source, effectName) {
  if (payload[field] === undefined) {
    return;
  }
  ...
  pushError(
    errors,
    'AUTHORED_INSTRUCTION_FIELD_REQUIRED',
    `${effectName}.${field} must be a non-empty string when provided.`,
    source,
    { effectName, field }
  );
}
...
function validateMetadataSet(payload, effectName, requireActor, errors, source) {
  ...
  validateOptionalTextField(payload, 'actor', errors, source, effectName);
  validateOptionalTextField(payload, 'player', errors, source, effectName);
  validateOptionalTextField(payload, 'roomRef', errors, source, effectName);
  ...
  for (const entry of effects) {
    ...
    if (effectNames.length !== 1) {
      pushError(
        ...
        'Each authored instruction entry must contain exactly one effect key.',
        source
      );
      continue;
    }
    ...
    if (!validator) {
      pushError(
        ...
        `Unsupported authored instruction: ${effectName}.`,
        source,
        { effectName }
      );
      continue;
