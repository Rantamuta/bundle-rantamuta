 ...
 *
 * This module is wired through `lib/runtime/command/render-dispatch.js` and is also
 * used directly by semantic-message focused tests.
 *
 * Design notes:
...
function resolveSelector(selector, context) {
  ...
  if (selector.selector === 'currentActor') {
    return context.currentActor || context.currentPlayer || null;
  }

  if (selector.selector === 'currentPlayer') {
    return context.currentPlayer || context.currentActor || null;
  }
...
function validateInstruction(instruction) {
  ...
  for (const key of participantKeys) {
    ...
    if (selector.selector === 'currentActor' || selector.selector === 'currentPlayer') {
      continue;
    }
...
function resolveParticipants(instruction, context) {
  if (context.currentActor && context.currentPlayer &&
    !sameEntity(context.currentActor, context.currentPlayer)) {
    return {
      ok: false,
      code: 'SEMANTIC_ACTOR_ALIAS_MISMATCH',
      message: 'currentActor and currentPlayer must resolve to the same entity identity.',
    };
  }
  ...
  if (!actor) {
    return {
      ok: false,
      code: 'SEMANTIC_ACTOR_UNRESOLVED',
...
function resolveToken(token, participants, recipient, objectText) {
  ...
  /** @type {(value: unknown) => { ok: true, value: string }} */
  const toValue = (value) => ({
    ok: true,
  ...
  if (key === 'target' || key === 'target.you') {
    ...
    return toValue(sameEntity(recipient, participants.target) ? 'you' : displayTargetName(participants.target));
  }
  if (key === 'target.name') {
    if (!participants.target) {
      return {
