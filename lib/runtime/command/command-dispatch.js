...
const Mutator = require('../mutation/mutator');
...
/** @typedef {import('../mutation/mutator').MutationPlan} MutationPlan */
...
function normalizeStructuredNpcIntent(intent) {
  ...
  if (Object.prototype.hasOwnProperty.call(intent, 'relationToken')) {
    if (intent.relationToken === null || intent.relationToken === undefined) {
      ...
      if (normalizedRelation && /\s/u.test(normalizedRelation)) {
        return dispatchFail('NPC_INTENT_INVALID', {
          reason: 'INVALID_RELATION_TOKEN',
          field: 'relationToken',
        });
 ...
 * @returns {boolean}
 */
function isItemLikeEntity(entity) {
  return !!entity &&
    typeof entity === 'object' &&
    !entity.isNpc &&
    !entity.isPlayer &&
    (
      Object.prototype.hasOwnProperty.call(entity, 'carriedBy')
...
/**
 * @param {*} entity
 * @param {{ verbId: string, role?: 'direct' | 'indirect' | null }} action
 * @returns {*}
 */
function resolveItemVerbPolicy(entity, action) {
  if (!isItemLikeEntity(entity) || !action || typeof action.verbId !== 'string' || action.verbId.length === 0) {
    return undefined;
  }
  ...
  const verbPolicy = verbs[action.verbId];
  if (
    action.role === 'indirect' &&
    verbPolicy &&
    typeof verbPolicy === 'object' &&
    Object.prototype.hasOwnProperty.call(verbPolicy, 'indirect')
  ) {
    return /** @type {Record<string, *>} */ (verbPolicy).indirect;
  }
...
function evaluateEntityPolicy(entity, action, context) {
  ...
  const itemVerbPolicy = resolveItemVerbPolicy(entity, action);
  if (itemVerbPolicy !== undefined) {
    return normalizePolicyOutcome(itemVerbPolicy);
  }

  if (isItemLikeEntity(entity)) {
    return null;
  }
