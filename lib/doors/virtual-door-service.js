...
/**
 * Determine whether an item definition exists for the given authored ref.
 * Validation uses this to verify `virtualDoor: <itemId>` facade bindings.
 *
 * @param {*} state
 * @param {string} itemRef
 * @returns {boolean}
 */
function hasItemDefinition(state, itemRef) {
  const itemFactory = state && typeof state === 'object'
    ? state.ItemFactory
    : null;
  const normalizedItemRef = normalizeRef(itemRef);
  if (!itemFactory || typeof itemFactory !== 'object' || !normalizedItemRef) {
    return false;
  }

  if (typeof itemFactory.getDefinition === 'function') {
    if (itemFactory.getDefinition(itemRef)) {
      return true;
    }

    if (itemFactory.getDefinition(normalizedItemRef)) {
      return true;
    }
  }

  const entities = itemFactory.entities;
  if (entities && typeof entities.entries === 'function') {
    for (const [entityRef] of entities.entries()) {
      if (normalizeRef(entityRef) === normalizedItemRef) {
        return true;
      }
    }
  }
...
/**
 * Validate authored VirtualDoor facade bindings.
 *
 * Policy:
 * - Explicit `virtualDoor: <itemId>` bindings are validated at bundle-validation
 *   time and do not throw during runtime/startup.
 * - Missing or invalid explicit facade item references are reported as validation
 *   errors so bundle authors must fix broken facade bindings before ship.
 *
 * @param {*} state
 * @returns {Array<{
 *  level: 'warn' | 'error',
 ...
 * }>}
 */
function validateVirtualDoorConfig(state) {
  const findings = [];
  const rooms = allRooms(state);
  const itemFactory = state && typeof state === 'object'
    ? state.ItemFactory
  ...
  if (!itemFactory || typeof itemFactory !== 'object') {
    findings.push({
      level: 'warn',
      code: 'VIRTUAL_DOOR_VALIDATION_SKIPPED',
      message: 'VirtualDoor validation skipped because ItemFactory is unavailable. Run bundle validation in --engine mode after bundles load.',
    });
    return findings;
  }

  for (const room of rooms) {
    const sourceRoomRef = room && typeof room.entityReference === 'string'
      ? room.entityReference
      : '';

    for (const exit of roomExits(room)) {
      const virtualDoorValue = exitVirtualDoorConfig(exit);
      if (typeof virtualDoorValue === 'undefined' || virtualDoorValue === false) {
        continue;
      }

      if (typeof virtualDoorValue !== 'string' || virtualDoorValue.trim().length === 0) {
        findings.push({
          level: 'error',
          code: 'VIRTUAL_DOOR_FACADE_INVALID',
          message: `VirtualDoor facade binding on "${sourceRoomRef}" exit "${String(exit && exit.direction || '')}" must be either false or a non-empty item id string. Update rooms.yml: set virtualDoor: false to opt out, remove virtualDoor to keep default pairing, or set virtualDoor to a valid item ref from this area's items.yml.`,
          detail: {
            roomRef: sourceRoomRef,
            direction: String(exit && exit.direction || ''),
            virtualDoor: virtualDoorValue,
            fix: 'Use virtualDoor: false | omit virtualDoor | virtualDoor: <existing itemRef>.',
          },
        });
        continue;
      }

      const facadeItemRef = virtualDoorValue.trim();
      if (!hasItemDefinition(state, facadeItemRef)) {
        findings.push({
          level: 'error',
          code: 'VIRTUAL_DOOR_FACADE_ITEM_MISSING',
          message: `VirtualDoor facade item "${facadeItemRef}" referenced on "${sourceRoomRef}" exit "${String(exit && exit.direction || '')}" does not exist. Define that item in areas/<area>/items.yml, or change/remove the virtualDoor value on this exit in rooms.yml.`,
          detail: {
            roomRef: sourceRoomRef,
            direction: String(exit && exit.direction || ''),
            virtualDoor: facadeItemRef,
            fix: 'Add the item definition or update virtualDoor on the exit.',
          },
        });
      }
    }
...
module.exports = {
  ...
  _validateVirtualDoorConfig: validateVirtualDoorConfig,
};
