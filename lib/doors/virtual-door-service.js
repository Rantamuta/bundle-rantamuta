...
function validateVirtualDoorConfig(state) {
  ...
  if (!itemFactory || typeof itemFactory !== 'object') {
    findings.push({
      ...
      message: 'VirtualDoor validation skipped because ItemFactory is unavailable.',
    });
    return findings;
  }
    ...
    for (const exit of roomExits(room)) {
      ...
      if (typeof virtualDoorValue !== 'string' || virtualDoorValue.trim().length === 0) {
        findings.push({
          ...
          message: `VirtualDoor facade binding on "${sourceRoomRef}" exit "${String(exit && exit.direction || '')}" must be a non-empty item id string or false.`,
          detail: {
      ...
      if (!hasItemDefinition(state, facadeItemRef)) {
        findings.push({
          ...
          message: `VirtualDoor facade item "${facadeItemRef}" referenced on "${sourceRoomRef}" exit "${String(exit && exit.direction || '')}" does not exist.`,
          detail: {
            roomRef: sourceRoomRef,
            direction: String(exit && exit.direction || ''),
