...
function getDeclaration(command, verbId) {
  ...
  declaration.compiledRules = precompiledRules || Syntax.compileSyntaxRules(verbId, declaration.syntaxRules);
  return declaration;
}
...
function legacySlotKindForRole(ruleConfig, role) {
  const scopes = scopeProfileForRole(ruleConfig, role);
  if (scopes.length === 1 && scopes[0].source === 'room.exits') {
    return 'EXIT';
  }
...
function resolveLegacyDeclaration(declaration, player, bodyTokens) {
  ...
  const value = {
    ...
    compiledRuleId: null,
    directSpan: legacy.directSpan,
    indirectSpan: legacy.indirectSpan,
    relationTokenRaw: legacy.relationTokenRaw,
    relationTokenCanonical: legacy.relationTokenCanonical,
    declaration,
    slots: [],
  ...
  if (ruleKey === 'intransitive' || ruleKey === 'relationOnly') {
    return { ok: true, value };
  }

  if (ruleKey === 'direct' || ruleKey === 'directIndirect') {
    const directScopes = scopeProfileForRole(ruleConfig, 'direct');
    const directSlotKind = legacySlotKindForRole(ruleConfig, 'direct');
    const directResult = bindRole('direct', directSlotKind, legacy.directSpan, player, directScopes);
    if (!directResult.ok) {
      return directResult;
    }
  ...
  if (ruleKey === 'indirect' || ruleKey === 'directIndirect') {
    ...
    const indirectSlotKind = legacySlotKindForRole(ruleConfig, 'indirect');
    const allowUnresolvedIndirect = ruleConfig.allowUnresolvedIndirect === true;
    const indirectResult = bindRole('indirect', indirectSlotKind, legacy.indirectSpan, player, indirectScopes);
    if (!indirectResult.ok) {
      if (allowUnresolvedIndirect) {
        value.indirectResolutionError = {
          code: indirectResult.error.code,
          details: indirectResult.error.details,
        };
        return { ok: true, value };
      }
...
function buildInterpretationValue(verbId, declaration, selectedRule, slotStates) {
  ...
  return {
    ...
    relationTokenRaw: selectedRule.canonicalRelationToken,
    relationTokenCanonical: selectedRule.canonicalRelationToken,
    directTarget: directSlot && directSlot.status === 'resolved' ? directSlot.selected : undefined,
    indirectTarget: indirectSlot && indirectSlot.status === 'resolved' ? indirectSlot.selected : undefined,
...
function resolveEntityContext(state, command, player, parsedInput) {
  ...
  for (const compiledRule of declaration.compiledRules || []) {
    const matched = Syntax.tryMatchRule(compiledRule, bodyTokens, (slot) => {
      ...
      return /** @type {{ status: 'resolved', selected: object, candidates: object[] } | { status: 'ambiguous', candidates: object[] } | { status: 'missing', candidates: object[] }} */ (resolveEntitySlot(slot, player, scopes));
    });
    ...
    const interpretation = buildInterpretationValue(verbId, declaration, compiledRule, matched.slotStates);
  ...
  if (Object.keys(declaration.rules || {}).length > 0) {
    return /** @type {{ ok: true, value: Record<string, *> } | { ok: false, error: { code: string, message?: string, details?: Record<string, *> } }} */ (
      resolveLegacyDeclaration(declaration, player, bodyTokens)
    );
  }
