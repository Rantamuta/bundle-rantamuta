...
function toExitCandidate(exit, index) {
  ...
  candidate.planDirect = (actor, verbId, context) => {
    ...
    if (GoExitHookHelper.contributionRequestsReplaceSuccess(authoredContribution)) {
      ...
      return Array.isArray(strippedFallback)
        ? [...strippedFallback, authoredContribution]
        : [strippedFallback, authoredContribution];
    }
    ...
    return Array.isArray(fallbackContribution)
      ? [...fallbackContribution, authoredContribution]
      : [fallbackContribution, authoredContribution];
  };
