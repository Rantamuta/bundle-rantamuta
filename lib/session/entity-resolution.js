...
const RULE_KEYS = new Set(['intransitive', 'direct', 'indirect', 'directIndirect', 'literal', 'literalIndirect', 'relationOnly']);
const RELATION_BEARING_RULES = new Set(['indirect', 'directIndirect', 'literalIndirect', 'relationOnly']);
 ...
 * @returns {boolean}
 */
function hasQuotedSpanToken(span) {
  return span.some(token => {
...
function selectRuleKey(declaration, shape) {
  switch (shape) {
    case 'intransitive':
      ...
      if (hasRule(declaration, 'direct') || hasRule(declaration, 'directIndirect') || hasRule(declaration, 'literal') || hasRule(declaration, 'literalIndirect')) {
    ...
    case 'directOnly':
      ...
      if (hasRule(declaration, 'literal')) {
        return { ok: true, ruleKey: 'literal' };
      }
      if (hasRule(declaration, 'directIndirect') || hasRule(declaration, 'literalIndirect')) {
    ...
    case 'directIndirect':
      ...
      if (hasRule(declaration, 'literalIndirect')) {
        return { ok: true, ruleKey: 'literalIndirect' };
      }
      return { ok: false, error: { code: 'FORM_NOT_SUPPORTED' } };
 ...
 * `room.npcs`, `room.exits`) returns deterministic candidates without mutating game state.
 *
 * @param {*} player
 * @param {string} source
...
function readScopeItems(player, source) {
  switch (source) {
    ...
    case 'room.players':
      return Helper.getRoomPlayers(player);
    case 'room.npcs':
...
function toExitCandidate(exit, index) {
  ...
  const authoredCanDirect = typeof exit.canDirect === 'function'
    ? exit.canDirect
    : null;
  const authoredPlanDirect = typeof exit.planDirect === 'function'
    ? exit.planDirect
    : null;
  ...
  candidate.canDirect = (actor, verbId, context) => {
    ...
    return authoredCanDirect
      ? authoredCanDirect(actor, verbId, context)
      : null;
  };
...
/**
 * @param {*} fallbackContribution
 * @param {*} authoredContribution
 * @returns {*}
 */
...
function resolveEntityContext(state, command, player, parsedInput) {
  ...
  if (!selectedRule.ok) {
    return /** @type {EntityResolutionFailure} */ (selectedRule);
  }
  ...
  if (RELATION_BEARING_RULES.has(ruleKey)) {
    ...
    if (hasQuotedSpanToken(indirectSpan)) {
      return {
        ok: false,
        error: {
  ...
  if (ruleKey === 'direct' || ruleKey === 'directIndirect') {
    ...
    if (!directResult.ok) {
      return /** @type {EntityResolutionFailure} */ (directResult);
    }

    value.directTarget = directResult.target;
  }

  if (ruleKey === 'indirect' || ruleKey === 'directIndirect' || ruleKey === 'literalIndirect') {
    ...
    if (indirectResult.ok === false) {
      ...
      return /** @type {EntityResolutionFailure} */ (indirectResult);
    }
