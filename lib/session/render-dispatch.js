...
function resolveSemanticOthers(context, actor, target) {
  const sourceRoom = currentRoom(context.player);
  const roomTargets = broadcastTargets(sourceRoom);
  /** @type {Array<*>} */
  const others = [];
  const seen = new Set();
  const actorSocket = actor && typeof actor === 'object' ? actor.socket : null;
  const targetSocket = target && typeof target === 'object' ? target.socket : null;
  ...
  for (const candidate of roomTargets) {
    ...
    if (targetSocket && candidateSocket && candidateSocket === targetSocket) {
      continue;
    }
...
function executeSemanticEventInstruction(context, instruction) {
  ...
  if (!actor || typeof actor !== 'object') {
    throw new TypeError('render.semanticEvent requires an active player.');
  }
