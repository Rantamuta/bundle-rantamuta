 ...
 * 4. Bubble (reaction)
 *    - Collect reaction contributions in bubble order:
 *      direct -> indirect -> player -> room -> area -> quest system -> world.
 *    - The reactions are semantic messages.
 ...
 *    - Render reaction (bubble) messages in contribution order (direct -> indirect -> player -> room -> area -> quest system -> world).
...
/**
 * @typedef {{ lines?: string[], instructions?: Array<*> }} CommandRenderPayload
 */
 ...
 * @param {Array<*>} renderInstructions
 * @param {{ operationsRejected: number }} diagnostics
 */
function consumeBubbleContribution(candidate, renderLines, renderInstructions, diagnostics) {
  if (!candidate) {
    return;
  }
  ...
  if (Array.isArray(candidate)) {
    for (const entry of candidate) {
      consumeBubbleContribution(entry, renderLines, renderInstructions, diagnostics);
    }
    return;
  }
  ...
  const hasRenderInstructions = !!render && Array.isArray(render.instructions);
  ...
  if (hasRenderInstructions && render) {
    renderInstructions.push(...render.instructions);
  }

  if (render && Object.prototype.hasOwnProperty.call(render, 'instructions') && !hasRenderInstructions) {
    Logger.error('Bubble contribution provided invalid render.instructions payload (expected array). Contribution ignored.');
  }
 ...
 * @param {*} command
 * @param {Record<string, *>} context
 * @returns {{ renderLines: string[], renderInstructions: Array<*>, operationsRejected: number }}
 */
function collectBubbleContributions(command, context) {
  ...
  for (const reaction of reactions) {
    ...
    consumeBubbleContribution(result, renderLines, renderInstructions, diagnostics);
  }
 ...
 * @returns {Array<*>}
 */
function extractRenderInstructions(renderPayload, sourceLabel) {
  if (!renderPayload || typeof renderPayload !== 'object') {
    return [];
  }

  const instructions = renderPayload.instructions;
  if (instructions === undefined) {
    return [];
  }

  if (!Array.isArray(instructions)) {
    Logger.error(`${sourceLabel} returned invalid render.instructions payload (expected array). Contribution ignored.`);
    return [];
  }
 ...
 * @param {Array<*>} [bubbleRenderInstructions]
 * @returns {{ linesRendered: number, instructionsAttempted: number, failures: number }}
 */
function renderSuccess(dispatchContext, success, bubbleRenderLines = [], bubbleRenderInstructions = []) {
  ...
  const targetRenderInstructions = extractRenderInstructions(render, 'Command');
  const instructions = [
    ...targetRenderInstructions,
    ...(Array.isArray(bubbleRenderInstructions) ? bubbleRenderInstructions : []),
  ];
  const instructionStats = RenderDispatch.executeRenderInstructions(dispatchContext, instructions);
  ...
  try {
    ...
    if (!entityResolution.ok) {
      ...
      trace.phases.entityResolution = {
        ok: false,
        code: resolutionFailure.error && resolutionFailure.error.code ? resolutionFailure.error.code : 'FORM_NOT_SUPPORTED',
        details: resolutionFailure.error && resolutionFailure.error.details ? resolutionFailure.error.details : undefined,
      };
      setFailureOutcome(
        trace,
        'entityResolution',
    ...
    if (isCommandSuccessResult(result)) {
      ...
      trace.phases.bubble = {
        ...
        renderInstructionsAdded: bubbleContributions.renderInstructions.length,
      };
      try {
      ...
      const renderStats = renderSuccess(
        ...
        result,
        bubbleContributions.renderLines,
        bubbleContributions.renderInstructions
      );
      trace.phases.render = {
        ok: renderStats.failures === 0,
        linesRendered: renderStats.linesRendered,
        instructionsAttempted: renderStats.instructionsAttempted,
