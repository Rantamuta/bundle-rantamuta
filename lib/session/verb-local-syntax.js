 ...
 * @returns {string[]}
 */
function legacyRulesToSyntaxRules(declaration) {
  ...
  if (Object.prototype.hasOwnProperty.call(rules, 'intransitive')) {
    syntaxRules.push('(empty)');
  }

  if (Object.prototype.hasOwnProperty.call(rules, 'direct')) {
    syntaxRules.push('ENTITY');
  }

  if (Object.prototype.hasOwnProperty.call(rules, 'indirect')) {
    const accepted = normalizeAcceptedRelations(rules.indirect && rules.indirect.acceptedRelations);
    for (const relation of accepted) {
      syntaxRules.push(`${relation} ENTITY`);
    }
  }

  if (Object.prototype.hasOwnProperty.call(rules, 'directIndirect')) {
    const accepted = normalizeAcceptedRelations(rules.directIndirect && rules.directIndirect.acceptedRelations);
    for (const relation of accepted) {
      syntaxRules.push(`ENTITY ${relation} ENTITY`);
    }
  }

  if (Object.prototype.hasOwnProperty.call(rules, 'relationOnly')) {
    const accepted = normalizeAcceptedRelations(rules.relationOnly && rules.relationOnly.acceptedRelations);
    for (const relation of accepted) {
      syntaxRules.push(relation);
    }
  }
...
function normalizeAcceptedRelations(value) {
  if (!Array.isArray(value)) {
    return [];
  }

  return [...new Set(value.map(canonicalizeRelation).filter(Boolean))];
}
