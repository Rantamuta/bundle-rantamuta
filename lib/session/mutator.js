...
/**
 * @typedef {NoopInstruction | TransferItemInstruction | MovePlayerInstruction | DoorMutationInstruction | OpenDoorInstruction | CloseAndLockDoorInstruction | SetPlayerMetadataInstruction | SetRoomFlagInstruction | SetAreaMetadataInstruction} MutationInstruction
 */
...
function applySetRoomFlagInstruction(state, instruction) {
  ...
  const hadFlagsProperty = Object.prototype.hasOwnProperty.call(metadata, 'flags');
  const previousFlagsRoot = metadata.flags;
  const hadFlagsObject = isObjectRecord(metadata.flags);
  if (!hadFlagsObject) {
    metadata.flags = {};
  }

  const flags = /** @type {Record<string, *>} */ (metadata.flags);
  const hadFlag = Object.prototype.hasOwnProperty.call(flags, key);
  const previousFlagValue = flags[key];
  ...
  return () => {
    ...
    if (!hadFlagsObject) {
      const currentFlagsRoot = metadata.flags;
      if (currentFlagsRoot === flags && Object.keys(flags).length === 0) {
        if (hadFlagsProperty) {
          metadata.flags = previousFlagsRoot;
        } else {
