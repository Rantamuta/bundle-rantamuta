 ...
 * @returns {function(): void}
 */
function applyCloseDoorInstruction(state, instruction) {
  const actor = instruction && instruction.actor && typeof instruction.actor === 'object'
    ? instruction.actor
    : null;
  if (!actor) {
  ...
  if (hasRoomRef) {
    destination = resolveRoomByRef(state, instruction.roomRef.trim());
    if (!destination) {
      Logger.warn(`doorMutation(close): roomRef "${instruction.roomRef}" was not found. Instruction ignored.`);
      return () => { };
    }
  } else if (hasDirection) {
    const exit = resolveExitByDirection(actor, instruction.direction);
    if (!exit) {
      Logger.warn(`doorMutation(close): direction "${instruction.direction}" was not found. Instruction ignored.`);
      return () => { };
    }
    ...
    const roomRef = typeof exit.roomId === 'string' && exit.roomId.trim().length > 0
      ...
      : '';
    if (!roomRef) {
      Logger.warn(`doorMutation(close): direction "${instruction.direction}" did not resolve to a roomId. Instruction ignored.`);
      return () => { };
    }

    destination = resolveRoomByRef(state, roomRef);
    if (!destination) {
      Logger.warn(`doorMutation(close): resolved roomRef "${roomRef}" for direction "${instruction.direction}" was not found. Instruction ignored.`);
      return () => { };
    }
  } else {
    Logger.warn('doorMutation(close): instruction requires either roomRef or direction. Instruction ignored.');
    return () => { };
  }

  if (typeof destination.getDoor !== 'function') {
    Logger.warn('doorMutation(close): destination room does not support door operations. Instruction ignored.');
    return () => { };
  }
 ...
 * @returns {function(): void}
 */
function applyUnlockDoorInstruction(state, instruction) {
  const actor = instruction && instruction.actor && typeof instruction.actor === 'object'
    ? instruction.actor
    : null;
  if (!actor) {
  ...
  if (hasRoomRef) {
    destination = resolveRoomByRef(state, instruction.roomRef.trim());
    if (!destination) {
      Logger.warn(`doorMutation(unlock): roomRef "${instruction.roomRef}" was not found. Instruction ignored.`);
      return () => { };
    }
  } else if (hasDirection) {
    const exit = resolveExitByDirection(actor, instruction.direction);
    if (!exit) {
      Logger.warn(`doorMutation(unlock): direction "${instruction.direction}" was not found. Instruction ignored.`);
      return () => { };
    }
    ...
    const roomRef = typeof exit.roomId === 'string' && exit.roomId.trim().length > 0
      ...
      : '';
    if (!roomRef) {
      Logger.warn(`doorMutation(unlock): direction "${instruction.direction}" did not resolve to a roomId. Instruction ignored.`);
      return () => { };
    }

    destination = resolveRoomByRef(state, roomRef);
    if (!destination) {
      Logger.warn(`doorMutation(unlock): resolved roomRef "${roomRef}" for direction "${instruction.direction}" was not found. Instruction ignored.`);
      return () => { };
    }
  } else {
    Logger.warn('doorMutation(unlock): instruction requires either roomRef or direction. Instruction ignored.');
    return () => { };
  }

  if (typeof destination.getDoor !== 'function') {
    Logger.warn('doorMutation(unlock): destination room does not support door operations. Instruction ignored.');
    return () => { };
  }
 ...
 * @returns {function(): void}
 */
function applyDoorMutationInstruction(state, instruction) {
  ...
  switch (instruction.mutation) {
    case 'open':
    case 'unlockAndOpen':
      return applyOpenDoorInstruction(state, instruction);

    case 'close':
      return applyCloseDoorInstruction(state, instruction);

    case 'unlock':
      return applyUnlockDoorInstruction(state, instruction);

    case 'closeAndLock':
      return applyCloseAndLockDoorInstruction(state, instruction);

    default:
      throw new RangeError(`Unsupported doorMutation value "${instruction.mutation}".`);
  }
}
