 ...
 * @returns {function(): void}
 */
function applySetRoomMetadataInstruction(instruction) {
 ...
 * @returns {function(): void}
 */
function applySetWorldMetadataInstruction(state, instruction) {
 ...
 * @returns {function(): void}
 */
function applyDeleteRoomMetadataInstruction(instruction) {
  const actor = instruction && instruction.actor;
  const room = actor && typeof actor === 'object' && actor.room && typeof actor.room === 'object'
    ? actor.room
    : null;
 ...
 * @returns {function(): void}
 */
function applyDeleteWorldMetadataInstruction(state, instruction) {
  const segments = parseMetadataPathWithPattern(instruction && instruction.key, METADATA_SEGMENT_PATTERN);
  if (!segments) {
...
function applyMutationInstruction(state, instruction) {
  ...
  switch (instruction.type) {
    ...
    case 'setRoomMetadata': return applySetRoomMetadataInstruction(instruction);
    case 'setAreaMetadata': return applySetAreaMetadataInstruction(instruction);
    case 'setWorldMetadata': return applySetWorldMetadataInstruction(state, instruction);
    case 'deleteRoomMetadata': return applyDeleteRoomMetadataInstruction(instruction);
    case 'deleteAreaMetadata': return applyDeleteAreaMetadataInstruction(instruction);
    case 'deleteWorldMetadata': return applyDeleteWorldMetadataInstruction(state, instruction);
