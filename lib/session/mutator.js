...
function applyTransferItemInstruction({ item, from, to }) {
  ...
  logMutationVerbose('transferItem', {
    item: entityRef(item),
    from: entityRef(from),
    to: entityRef(to),
  });
 ...
 */
function logMutationVerbose(op, fields) {
  ...
  Logger.verbose(`MUTATOR apply ${op}${parts.length ? ` ${parts.join(' ')}` : ''}`);
}
...
function applyMovePlayerInstruction({ player, toRoom, direction, suppressRoomBroadcast }) {
  ...
  logMutationVerbose('movePlayer', {
    actor: entityRef(player) || actorName,
    from: entityRef(previousRoom),
    to: entityRef(toRoom),
...
function applyDoorMutationInstruction(state, instruction, warningPrefixOverride) {
  ...
  logMutationVerbose('doorMutation', {
    mutation: instruction.mutation,
    actor: entityRef(actor),
    fromRoom: entityRef(fromRoom),
    roomRef: typeof instruction.roomRef === 'string' ? instruction.roomRef.trim() : '',
    direction: typeof instruction.direction === 'string' ? normalizeDirection(instruction.direction) : '',
    changed: result.changed === true ? 'true' : 'false',
  });
