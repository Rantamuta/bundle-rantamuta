...
function resolveToken(token, participants, recipient, objectText) {
  ...
  if (key.startsWith('verb:')) {
    ...
    return { ok: true, value: inflectVerb(baseVerb, shouldInflect) };
  }

  if (key === 'actor' || key === 'actor.you') {
    return { ok: true, value: sameEntity(recipient, participants.actor) ? 'You' : displayName(participants.actor) };
  }
  if (key === 'actor.name') {
    return { ok: true, value: displayName(participants.actor) };
  }
  if (key === 'actor.poss') {
    // Recipient-relative second-person path takes precedence.
    if (sameEntity(recipient, participants.actor)) {
      return { ok: true, value: 'your' };
    }
    return { ok: true, value: thirdPersonPossessive(participants.actor) };
  }
  if (key === 'actor.name_poss') {
    return { ok: true, value: possessive(displayName(participants.actor)) };
  }
  if (key === 'actor.refl') {
    if (sameEntity(recipient, participants.actor)) {
      return { ok: true, value: 'yourself' };
    }
    return { ok: true, value: thirdPersonReflexive(participants.actor) };
  }
  ...
  if (key === 'target' || key === 'target.you') {
    ...
    return { ok: true, value: sameEntity(recipient, participants.target) ? 'you' : displayName(participants.target) };
  }
  if (key === 'target.name') {
    if (!participants.target) {
      return {
    ...
    return { ok: true, value: displayName(participants.target) };
  }
  if (key === 'target.poss') {
    if (!participants.target) {
      return {
    ...
    return { ok: true, value: thirdPersonPossessive(participants.target) };
  }
  if (key === 'target.name_poss') {
    ...
    return { ok: true, value: possessive(displayName(participants.target)) };
  }
  if (key === 'target.refl') {
    if (!participants.target) {
      return {
    ...
    return { ok: true, value: thirdPersonReflexive(participants.target) };
  }
  ...
  if (key === 'direct') {
    ...
    if (typeof objectText.direct === 'string' && normalizeText(objectText.direct)) {
      return { ok: true, value: normalizeText(objectText.direct) };
    }
    ...
    return { ok: true, value: displayName(participants.direct) };
  }

  if (key === 'indirect') {
    // Explicit object text (if provided) wins over entity label lookup.
    if (typeof objectText.indirect === 'string' && normalizeText(objectText.indirect)) {
      return { ok: true, value: normalizeText(objectText.indirect) };
    }
    ...
    return { ok: true, value: displayName(participants.indirect) };
  }
