...
function parseInput(actorInput) {
  ...
  if (!tokens.length || !tokens[0]) {
    return { actorInput: raw, normalizedInput };
  }
  ...
  if (inTransitive) {
    ...
    return { actorInput: raw, normalizedInput, intentToken: normalizedInput };
  }

  // If there is no relation token, then the intent is the first token and the primary target is everything after it.
  if (!relationToken) return { actorInput: raw, normalizedInput, intentToken, primaryTargetSpan };
