...
/**
 * Resolve outbound door state from current room to a direction.
 *
 * @param {{ room: *, area: *, world: * }} scope
 * @param {string} direction
 * @returns {* | null}
 */
function outboundDoor(scope, direction) {
  ...
  const destination = resolveRoom(scope, exit.roomId);
  if (!destination || typeof destination.getDoor !== 'function') {
    return null;
  }
...
/**
 * Resolve inbound door state on current room from a source direction.
 *
 * @param {{ room: *, area: *, world: * }} scope
 * @param {string} direction
 * @returns {* | null}
 */
function inboundDoor(scope, direction) {
  const room = scope.room && typeof scope.room === 'object'
    ? scope.room
    : null;
  if (!room) {
    return null;
  ...
  const exit = findRoomExit(room, direction);
  if (!exit || typeof exit.roomId !== 'string' || exit.roomId.trim().length === 0) {
    return null;
  }

  const sourceRoom = resolveRoom(scope, exit.roomId);
  if (!sourceRoom || typeof room.getDoor !== 'function') {
    return null;
  }

  const door = room.getDoor(sourceRoom);
  return door && typeof door === 'object' ? door : null;
}
 ...
 *
 * @param {{ actor: *, room: *, area: *, world: *, entity: *, currentContainer: * }} scope
 * @returns {{
 ...
 *  outboundDoorOpen: (direction: string) => boolean,
 *  inboundDoorOpen: (direction: string) => boolean,
 * }}
 */
function createQueryFacade(scope) {
  const facade = {
    ...
    // NOTE: Provisional door query naming for current POC.
    // TODO: Revisit and normalize these APIs after the full Door plan/spec is finalized.
    outboundDoorOpen: direction => {
      const door = outboundDoor(scope, direction);
      if (!door) {
        return false;
      }

      return door.closed !== true && door.locked !== true;
    },

    inboundDoorOpen: direction => {
      const door = inboundDoor(scope, direction);
      if (!door) {
        return false;
      }
