    ...
    getWorldMetadata: key => {
      ...
      return readMetadataValue(metadata, key, details => {
        const pathLabel = details.pathPrefix
          ...
          : details.segment;
        warnUnresolvable(
          `PREDICATE_QUERY_METADATA_KEY_COLLISION:getWorldMetadata:world:${String(pathLabel || '').toLowerCase()}`,
          `PREDICATE_QUERY_METADATA_KEY_COLLISION: Predicate query q.getWorldMetadata("${String(key || '')}") found case-colliding metadata keys at "${pathLabel}": [${details.matchedKeys.join(', ')}]. Using last match "${details.matchedKeys[details.matchedKeys.length - 1]}".`
        );
      });
    },
