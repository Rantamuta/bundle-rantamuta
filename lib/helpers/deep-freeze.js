...
/**
 * @fileoverview
 * Deep-freeze utility that recursively freezes supported objects/functions/arrays
 * and returns a deep-readonly typed view.
 *
 * Behavior summary:
 * - cycle-safe recursion via WeakSet
 * - traverses own data properties only (getters are never invoked)
 * - primitives are returned unchanged
 * - ArrayBuffer views (for example, Buffer/TypedArray) are not frozen
 * - Map/Set values are rejected as unsupported
 */
