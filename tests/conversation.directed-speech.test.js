...
describe('bundle-rantamuta conversation directed speech facade', function () {
  ...
  it('normalizes spoken event casing before conversation lookup', function () {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'conversation-directed-speech-'));
    writeConversation(tempRoot, [
      'id: actor_planner',
      'initial: greeting',
  ...
  it('falls through quietly when a well-formed live guard is false', function () {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'conversation-directed-speech-'));
    writeConversation(tempRoot, [
      'id: actor_planner',
      'initial: greeting',
  ...
  it('uses authored default when an exact guarded event is unavailable', function () {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'conversation-directed-speech-'));
    writeConversation(tempRoot, [
      'id: actor_planner',
      'initial: greeting',
  ...
  it('uses guarded authored default for unmatched spoken actions', function () {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'conversation-directed-speech-'));
    writeConversation(tempRoot, [
      'id: actor_planner',
      'initial: greeting',
